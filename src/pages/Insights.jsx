  ...
  return (
    <div className={`pt-20 bg-[var(--bg-primary)] min-h-screen text-[var(--text-primary)] relative transition-colors duration-700 ${lang === 'ar' ? 'font-arabic' : ''}`}>
      ...
      <section className="relative py-40 md:py-60 overflow-hidden z-10">
        <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-20 text-center">
          ...
          >
            ...
            <h1 className={`text-7xl md:text-[11rem] font-black mb-12 leading-[0.8] uppercase tracking-tighter ${lang === 'ar' ? 'font-arabic' : ''}`}>
            ...
            <p className="text-2xl md:text-3xl text-[var(--text-primary)] opacity-40 leading-relaxed font-light max-w-3xl mx-auto">
              {lang === 'en' 
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
              ...
              >
                 ...
                 <h2 className={`text-5xl md:text-7xl font-black mb-10 uppercase tracking-tighter leading-none ${lang === 'ar' ? 'font-arabic' : ''}`}>
                 ...
                 <div className={`flex flex-col sm:flex-row gap-8 mb-16 ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
                    <a href="https://sellingdubai.substack.com" target="_blank" rel="noopener noreferrer" className="btn-premium py-5 px-10 border-none bg-[var(--accent-gold)] text-[var(--bg-primary)]">
                       {lang === 'en' ? 'Browse Full Archive' : 'تصفح الأرشيف الكامل'} <ExternalLink size={14} className={`inline ${lang === 'ar' ? 'mr-2 rotate-180' : 'ml-2'}`} />
                    </a>
                    <div className="flex flex-col gap-4">
                       <div className={`flex items-center gap-4 group ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
              ...
              >
                 ...
                 <div className={`absolute -bottom-10 ${lang === 'ar' ? '-left-10' : '-right-10'} w-40 h-40 bg-[var(--accent-gold)]/10 rounded-full blur-3xl -z-10`} />
              </motion.div>
           </div>
        </div>
      </section>
      ...
      <section className="py-60 relative z-10 transition-colors duration-700">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
           <div className={`flex flex-col md:flex-row justify-between items-end mb-32 gap-10 ${lang === 'ar' ? 'md:flex-row-reverse' : ''}`}>
              <div className={lang === 'ar' ? 'text-right' : 'text-left'}>
                 ...
                 <h2 className={`text-5xl md:text-8xl font-black uppercase tracking-tighter ${lang === 'ar' ? 'font-arabic' : ''}`}>
              ...
              <p className={`opacity-30 max-w-md text-sm leading-relaxed font-light mb-4 ${lang === 'ar' ? 'text-right' : 'text-left'}`}>
                 {lang === 'en' 
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              {articles.map((article, i) => (
                <motion.a
                  ...
                  whileHover={{ y: -15 }}
                  className={`premium-card p-12 md:p-16 flex flex-col justify-between group h-[450px] cursor-pointer border-[var(--border-color)] bg-[var(--text-primary)]/[0.02] ${lang === 'ar' ? 'text-right' : 'text-left'}`}
                >
                   <div>
                   ...
                   <div className={`flex items-center justify-between pt-10 border-t border-[var(--border-color)] ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
                      ...
                      <div className={`flex items-center gap-3 font-black uppercase tracking-[0.3em] text-[9px] group-hover:text-[var(--accent-gold)] transition-all ${lang === 'ar' ? 'flex-row-reverse' : ''}`}>
                         {lang === 'en' ? 'READ STORY' : 'اقرأ القصة'} <ChevronRight size={14} className={lang === 'ar' ? 'rotate-180' : ''} />
                      </div>
                   </div>
                </motion.a>
              ))}
